...
int main() {
    ...
    // -------------------- 顶点数据 & VAO/VBO/EBO --------------------
    glGenVertexArrays(1, &VAO); // 生成 VAO（顶点数组对象）
    glGenBuffers(1, &VBO);      // 生成 VBO（顶点缓冲对象）
    glGenBuffers(1, &EBO);      // 生成 EBO（索引缓冲对象）

    glBindVertexArray(VAO);     // 绑定 VAO

    // 绑定并传输索引数据到 EBO
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO); // 绑定 EBO
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW); // 将索引数据复制到 EBO
